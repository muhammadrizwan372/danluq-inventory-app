    ...
    <div>
      ...
      {sales && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Revenue ({days}d)</p><p className="text-2xl font-bold text-gray-900">Rs.{sales.total_revenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div><div className="h-12 w-12 rounded-xl bg-green-600 flex items-center justify-center"><DollarSign className="h-6 w-6 text-white" /></div></div></div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5"><div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">Avg Order Value</p><p className="text-2xl font-bold text-gray-900">Rs.{sales.average_order_value.toFixed(2)}</p></div><div className="h-12 w-12 rounded-xl bg-purple-600 flex items-center justify-center"><BarChart3 className="h-6 w-6 text-white" /></div></div></div>
        </div>
      )}
      ...
      {stock && (
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5"><p className="text-sm text-gray-500">Stock Value</p><p className="text-xl font-bold">Rs.{stock.total_stock_value.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div>
        ...
        {topSelling && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-gray-50 border-b border-gray-200"><tr><th className={TH}>Product</th><th className={TH}>SKU</th><th className={TH}>Sold</th><th className={TH}>Revenue</th></tr></thead>
              <tbody>{topSelling.products.length === 0 ? <tr><td colSpan={4} className="text-center text-gray-400 py-8">No data yet</td></tr> : topSelling.products.map((p) => (<tr key={p.id} className="hover:bg-gray-50"><td className={`${TD} font-medium`}>{p.name}</td><td className={`${TD} text-gray-500`}>{p.sku}</td><td className={TD}>{p.total_sold}</td><td className={`${TD} font-medium`}>Rs.{p.total_revenue.toFixed(2)}</td></tr>))}</tbody>
            </table></div>
          </div>
        )}
        {salesByCategory && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-gray-50 border-b border-gray-200"><tr><th className={TH}>Category</th><th className={TH}>Units Sold</th><th className={TH}>Revenue</th></tr></thead>
              <tbody>{salesByCategory.categories.length === 0 ? <tr><td colSpan={3} className="text-center text-gray-400 py-8">No data yet</td></tr> : salesByCategory.categories.map((c, i) => (<tr key={i} className="hover:bg-gray-50"><td className={`${TD} font-medium`}>{c.category}</td><td className={TD}>{c.total_sold}</td><td className={`${TD} font-medium`}>Rs.{c.total_revenue.toFixed(2)}</td></tr>))}</tbody>
            </table></div>
          </div>
        )}
      </div>
